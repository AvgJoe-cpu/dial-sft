...
from src.ar2mlm.mlm_scheduler import LinearAlphaScheduler, CosineAlphaScheduler, BaseAlphaScheduler
...
class MDLMTrainer(Trainer):
    def __init__(
        ...
        **kwargs,
        ...
        if tok is None:
            raise ValueError(
                "MDLMTrainer requires a tokenizer via `processing_class`."
            )
        if getattr(tok, "padding_side", None) != "right":
            raise ValueError(
                f"padding_side must be 'right', got {tok.padding_side!r}."
            )
...
if __name__ == "__main__":
    ...
    # Ensure your tokenizer has a mask token for MDLM
    if tokenizer.mask_token_id is None:
        raise ValueError("Tokenizer has no mask_token_id")

    from datasets import Dataset
    train_ds = Dataset.from_dict({
        "input_ids": [[101, 2023, 2003, 103, 102] for _ in range(1000)],
        "labels":    [[101, 2023, 2003, 103, 102] for _ in range(1000)],
    })
    eval_ds = Dataset.from_dict({
        "input_ids": [[101, 2023, 2003, 103, 102] for _ in range(100)],
        "labels":    [[101, 2023, 2003, 103, 102] for _ in range(100)],
    })

    # Collator: pads to longest in batch; labels must stay aligned with input_ids
    ...
    # Metrics & trainer
    metric_fn = NLLPPLMetricComputer()
    trainer = MDLMTrainer(
        model=model,
        args=args,
