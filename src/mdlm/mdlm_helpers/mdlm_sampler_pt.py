...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmp:
        download_bucket_files(
            ...
            files=[
                ("model.safetensors", f"{tmp}/model.safetensors"),
                ("modeling_mdlm.py", f"{tmp}/modeling_mdlm.py"),
                ("config.json", f"{tmp}/config.json"),
                ("configuration_mdlm.py", f"{tmp}/configuration_mdlm.py"),
            ],
        )

        model = AutoModelForMaskedLM.from_pretrained(tmp, trust_remote_code=True)
        ...
        print("any masks left:", (out == mask_index).any().item())
