...
def convert(model_name_or_path: str, output_dir: str, random_init: bool = False):
    ...
    src_model     = transformers.AutoModelForCausalLM.from_pretrained(model_name_or_path, revision="step2000",torch_dtype=torch.bfloat16)
    src_tokenizer = transformers.AutoTokenizer.from_pretrained(model_name_or_path , revision="step2000")
...
if __name__ == "__main__":
    convert(
        model_name_or_path=sys.argv[1] if len(sys.argv) > 1 else "EleutherAI/pythia-400m",
        output_dir=sys.argv[2]         if len(sys.argv) > 2 else "models-tmp/a2d-gpt-neox-400m",
    )