# SOFT REWRITE OF THE ORIGINAL MDLM MODELING CODE: https://huggingface.co/kuleshov-group/mdlm-owt
...
# THE MODEL CAN BE ACCESSED LIKE THIS- THE BUCKET CONTAINS THE ORIGINAL WEIGHTS
# ------------------------------------------------------------------------------------------------------------------------
# from huggingface_hub import download_bucket_files
# download_bucket_files(
#    "avgJo3/mdlm-owt-bucket",
#    files=[
#        ("model.safetensors", "./local/model.safetensors"),
#        ("modeling_mdlm.py", "./local/modeling_mdlm.py"),
#        ("config.json", "./local/config.json"),
#        ("configuration_mdlm.py", "./local/configuration_mdlm.py"),
#    ],
# )
#
...
class DDiTBlock(nn.Module):
    def __init__(
        self, hidden_dim, n_heads, cond_dim, mlp_ratio: int = 4, dropout: float = 0.1
    ):
        super().__init__()
        ...
        )  # PyTorch ≥ 2.1 supports `bias=False`
...
class DITBackbone(nn.Module):
    ...
    def forward(
        self, input_ids, sigma, attention_mask=None, output_hidden_states=False
    ):
